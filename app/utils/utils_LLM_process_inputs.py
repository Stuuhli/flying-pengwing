...
class DefaultRefineProgram(BasePydanticProgram):
    ...
    async def acall(self, *args: Any, **kwds: Any) -> StructuredRefineResponse:
        if self._output_cls is not None:
            ...
            logger.debug("Before LLM call, we have prompt: %s and kwarg: %s", self._prompt, kwds)
...
# Qwen3 rerank prompt processing
def qwen_rerank_preprocess(query:str, texts: List):
    ...
    updated_query=query_template.format(prefix=prefix, instruction=instruction, query=query)
    updated_docs=[document_template.format(doc=text, suffix= suffix) for text in texts]
    return updated_query, updated_docs